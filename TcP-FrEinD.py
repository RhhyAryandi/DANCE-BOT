            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip().startswith('/restart'):
                            try:
                                uid = response.Data.uid
                                chat_id = response.Data.Chat_ID
                                message = "[B][C][FF0000]🚪 Bot akan keluar dan restart otomatis..."
                                P = await SEndMsG(response.Data.chat_type, message, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                print("[*] /restart diterima — restart script di Termux...")
                                ...
                                print(f"[!] Gagal restart saat /leave: {e}")
...
async def MaiiiinE():
    Uid , Pw = '4233344155','JOBAYAR_CODX-7CGRAYEZ8'
